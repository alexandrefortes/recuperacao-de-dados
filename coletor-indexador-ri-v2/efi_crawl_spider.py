...
class TfidfCalculationPipeline:
    ...
    def close_spider(self, spider):
        ...
        with open('tfidf_index.json', 'w', encoding='utf-8') as f:
            json.dump(self.tfidf_index, f, ensure_ascii=False, indent=4)
    ...
    def process_item(self, item, spider):
        ...
        # Concatenar os campos relevantes para formar o texto completo a ser indexado
        text_content = f"{item['title']} {item['main_title']} {item['description']} {' '.join(item['keywords'])} {' '.join(item['content'])}"
    ...
    def parse_page(self, response):
        ...
        try:
            ...
            yield EfiItem(
                ...
                content=filtered_content  # já filtrado
            )
